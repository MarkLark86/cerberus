...
async def test_empty_schema():
    ...
    with pytest.raises(SchemaError, match=errors.SCHEMA_ERROR_MISSING):
        await validator({}, schema=None)


@mark.asyncio
async def test_bad_schema_type(validator):
...
@mark.asyncio
async def test_bad_schema_type_field(validator):
    field = 'foo'
...
@mark.asyncio
async def test_unknown_rule(validator):
...
@mark.asyncio
async def test_unknown_type(validator):
...
@mark.asyncio
async def test_bad_schema_definition(validator):
    field = 'name'
...
@mark.asyncio
async def test_bad_of_rules():
    schema = {'foo': {'anyof': {'type': 'string'}}}
    await assert_schema_error({}, schema)


@mark.asyncio
async def test_normalization_rules_are_invalid_in_of_rules():
    schema = {0: {'anyof': [{'coerce': lambda x: x}]}}
    await assert_schema_error({}, schema)


@mark.asyncio
async def test_anyof_allof_schema_validate():
    # make sure schema with 'anyof' and 'allof' constraints are checked
    # correctly
    schema = {
        'doc': {'type': 'dict', 'anyof': [{'schema': [{'param': {'type': 'number'}}]}]}
    }
    await assert_schema_error({'doc': 'this is my document'}, schema)
...
@mark.asyncio
async def test_validated_schema_cache():
